...
[private vulnerability reporting](https://github.com/Echo6Bravo/curated-aws-remediation-generator/security/advisories/new)
...
makes no network calls, and never modifies AWS.** It reads local JSON and writes local text files.
So the interesting failures are not the usual ones.
...
- **A generated artifact that acts on the wrong scope.** The highest-severity class. If you can
  make the tool emit a script or HCL file that targets a different account or region than the one
  it claims — defeating the per-account/per-region split, the `aws sts get-caller-identity`
  preflight, or `allowed_account_ids` — that is a critical finding. The failure mode is a *silent
  success* against the wrong resources.
...
- **A remediation misclassified as safer than it is** — an irreversible, cost-scaled, or
  availability-affecting change emitted under `--tier safest`.
...
- The consequences of running a `--tier caution` or `--tier all` artifact you chose to run. Those
  tiers require an explicit opt-in and the warnings are inline in the artifact.
...
The tool never asks for, stores, or transmits AWS or Tenable credentials. Findings you supply are
