...
- **An S3 Block Public Access recipe — Batch 4 of the AWS register (5 → 6 recipes).** `aws s3api
  ...
  default safety level emits.
- **`Recipe.critical_caveats` — a second authored-documentation channel that renders inline, marked
  `!!`.** The tool's `safety_tier` is *derived* from exactly four fields (`data_path_impact`,
  `needs_replacement`, `reversible`, `cost_impact`/`blocks_iac_destroy`), and **none of them can say
  "this withdraws access somebody depends on"**. Block Public Access is the case that exposed it: it is
  reversible, free and in-place, so every one of the four says `safest` — while applying it stops
  anonymous reads the moment it lands. Its banner said `SAFEST` and the warning that mattered lived in
  another file, contradicting the project's own stated rule that warnings stay next to what they warn
  about.

  `caveats` stays README-only, which is deliberate and load-bearing: repeating a paragraph of reference
  prose beside each of hundreds of commands is what makes comments most of the output.
  `critical_caveats` renders in **both** places, at all four render points — both shell generators and
  standalone HCL via `recipe_notes`, merged HCL blocks via `_merged_target_notes` (attributed
  `!! [policy title]` and deduplicated, since one merged block carries several policies), and the run
  README above the derived Safety section rather than below it, because listing it after the derived
  notes buries the only line those notes could not produce. The two authored tuples must be
  **disjoint** — `__post_init__` rejects a caveat present in both, so promoting one means moving it,
  not copying it, and the README-only assertion that proves relocation stays true.

  **Six of fourteen recipes across both clouds use the channel**, and five of them are Azure. That is
  the part worth reading: the first draft of the test that finds access-withdrawing recipes listed the
  S3 recipe's own phrases, matched it, matched **zero** Azure recipes, and so certified that Azure had
  no such recipe when **five of its eight are exactly that** — HTTPS-only, minimum TLS, cross-tenant
  replication, SFTP and local-user authentication all break a live caller and all derive to `safest`.
  A detector tuned until it passes is not a detector. Rewritten around the *consequence* rather than any
  recipe's wording and moved to `tests/conftest.py` so both clouds share one definition, with a
  `>= 5` floor in the Azure suite so a future narrowing fails loudly instead of reporting a clean bill
  of health. Its limitation is stated where it lives: it is a text search over prose and it fails open.
- **Ten new tests (22 with parametrization) and a fifth inline CI gate for the `!!` channel.** The
  tests assert it reaches script, HCL and README; that it renders *before* the command rather than
  merely somewhere in the file; that a promoter is `safest` and does not paraphrase a note the tier
  already derives; and that promotion raises rather than duplicates. Mutation-tested rather than
  trusted for passing — and that found a real self-referential bug in one of them: it interpolated
  `CRITICAL_CAVEAT_MARKER` into its own expected string, so emptying the constant satisfied the
  assertion while the output became indistinguishable from an ordinary comment. It now asserts the
  constant has substance *before* interpolating it. Eight mutations on the mechanism and five on the
  gate, all caught after that fix. The gate is provider-discovered with a `< 2` anti-vacuity exit and
  checks the `README.md` count in words, so the prose number cannot drift from the code.
...
- **The committed AWS sample now exercises the new recipe, and its input says why that bucket.** The
  ...
  output. The finding was added on `acme-web-assets-use1`, on that bucket deliberately: its name says it
  serves web assets, so it is precisely the case the critical caveat is about. AWS now reads 11 records
  / 9 usable / 7 remediations across 5 artifact files, and both example READMEs quote the rendered `!!`
  block verbatim from the committed output rather than from memory.
- **`comment_block` hangs continuation lines under `!!`, not only under `- ` bullets.** Without it a
  wrapped five-line caveat's tail sat flush with the ordinary notes below it, so the warning visually
  ended at its first line.
- **Eight stale claims corrected in `examples/README.md`, several found by measuring rather than
  reading.** The safety-level table was wrong in *both* rows — it said 1 written / 5 withheld and 6 / 0,
  where running both levels gives 2 / 5 and 7 / 0 — along with the record arithmetic, the per-file
  remediation counts, and the recipe count. `examples/README.azure.md` gains the same `!!` section,
  stating that 5 of 8 Azure recipes use the channel and that three of them appear in that sample, both
