...
**Azure is no longer on this list.** `azremgen` ships four curated recipes verified on all three
...
- **`verify` checks shape, not semantics — on all three axes.** It confirms that an operation, a
...
- **All three upstreams are now watched weekly** (`.github/workflows/drift-canary.yml`), against the
  *newest* published botocore, provider and runner-bundled CLI rather than pinned ones — a pinned
