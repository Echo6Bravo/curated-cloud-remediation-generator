...
emits commands that people run against production AWS accounts.
...
2. **API call and parameters** confirmed against the AWS service model (`service-2.json`) —
   the same source `remgen verify` reads. Confirm the operation name and every parameter's shape.
...
- `remgen verify` passes for the new recipe.
...
[ROADMAP.md](./ROADMAP.md): no boto3 generator, no live API adapter, no non-AWS clouds, and the
unresolved question of how to gate non-reversible remediations. If you want to change one of those,
argue the tradeoff rather than just supplying the code.
